...
function loadProfiles() {
  chrome.storage.local.get(['profiles', 'activeProfileIndex'], (data) => {
    ...
    directOpt.innerHTML = "➔ Direct Connection (No Proxy)";
    ...
    profiles.forEach((p, index) => {
      ...
      opt.value = index;
      opt.innerHTML = `${p.name} (socks5://${p.host}:${p.port})`;
...
deleteBtn.addEventListener('click', () => {
  ...
  chrome.storage.local.get(['profiles', 'activeProfileIndex'], (data) => {
    ...
    if (String(data.activeProfileIndex) === String(index)) {
      updateData.activeProfileIndex = "direct"; // در صورت حذف، به حالت دایرکت برگردد
